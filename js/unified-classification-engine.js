...
(function(root) {
  ...
  const SPELL_PROFILES = {
    ...
    'exevo mort ora': { label: 'Death Echo', type: 'attack', element: 'death', topology: 'area', vocation: 'sorcerer' },
  ...
  function resolveSingleTargetAaVocationTurn(turn, facts, context) {
    ...
    const defs = chosen.map(def => {
      ...
      return Object.assign({}, def, {
        deterministic: def.comp === actionComp
          ? { ok: true, reason: 'ek_physical_spell_not_hard_gated_by_intersection' }
          : validatePhysicalBlock(block),
  ...
  function classifyUnifiedParsed(server, local, options, bmDetection) {
    ...
    const turns = buildTurns(server.hits);
    ...
    const result = {
      version: VERSION,
      status: resolvedTurns.some(t => t.status === 'unresolved') ? 'partial' : 'resolved',
